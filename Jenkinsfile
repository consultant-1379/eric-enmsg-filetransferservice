...
pipeline {
    ...
    stages {
        ...
        stage('Checkout Cloud-Native SG Git Repository') {
            steps {
                git branch: 'master',
                     ...
                     url: '${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-filetransferservice'
                sh '''
                      git remote set-url origin --push https://${GERRIT_HTTP_CREDENTIALS_FUser}@${GERRIT_CENTRAL_HTTP_E2E}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-filetransferservice
                  '''
            }
        }
        stage('Helm Dep Up ') {
        ...
        stage('Merge values files') {
            steps{
                 script {
                     appconfig_values = sh (script: "ls ${WORKSPACE}/chart/eric-enmsg-filetransferservice/appconfig/ | grep values.yaml", returnStatus: true)
                     ...
                     sh '''
                         if git status | grep 'values.yaml' > /dev/null; then
                            git add chart/eric-enmsg-filetransferservice/values.yaml
