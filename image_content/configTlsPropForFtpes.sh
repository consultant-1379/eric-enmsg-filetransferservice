...
readonly _FTPESCONFIG_SCRIPT="/ericsson/enm/ftpes-config.sh"
...
#########################################################
# This method is used to read the sps vm instance from physical or cloud
#########################################################
get_service_instance() {
    if [ -f ${GLOBAL_PROPERTIES_FILE} ] ; then
        service_instance_name="$(getent hosts sps |awk '{print $1}'|sed -n "$1"p)";
    else
...
readSystemTlsEcimPibValue(){
    ...
    if [[ ! "$ECIM_PIB_VALUE" == *"TLS"* ]]; then
        get_service_instance "2"
        ECIM_PIB_VALUE=$("$_PIB_CONFIG_SCRIPT" read --app_server_address "$service_instance_name":8080 --name="$PIB_NAME"|tr -d "\t\n\r ")
    fi

   if [[ ! "$ECIM_PIB_VALUE" == *"TLS"* ]]; then
        error "Error connecting to sps"
        exit 1;
    fi
}
